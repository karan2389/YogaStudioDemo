...
export function AdminOverview() {
  ...
  useEffect(() => {
    const load = () =>
      setCounts({
        ...
        promotions: getDemoPromotions().length,
      });
    load();
  ...
  return <>
    <DashboardPageHeader eyebrow="Admin overview" title="Studio control centre" copy="Manage the people, yoga sessions and customer bookings behind the Ananda Yoga demo." />
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">{shortcuts.map(({ href, label, copy, icon: Icon, key }) => <Link key={href} href={href} className="group rounded-[1.25rem] border border-[#17362d]/10 bg-white p-5 shadow-[0_8px_28px_rgba(36,57,47,0.04)] transition hover:-translate-y-0.5"><span className="grid size-10 place-items-center rounded-full bg-[#e5ecec] text-[#3e5a62]"><Icon className="size-5" /></span><p className="mt-5 text-2xl font-semibold">{counts[key] ?? 0}</p><p className="mt-1 font-semibold">{label}</p><p className="mt-1 text-xs leading-5 text-[#65756e]">{copy}</p><ArrowRight className="mt-4 size-4 text-[#a65f3d] transition group-hover:translate-x-1" /></Link>)}</div>
