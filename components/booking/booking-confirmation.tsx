...
import { CalendarCheck, CalendarDays, Check, Clock3, ReceiptText, UserRound } from "lucide-react";
  ...
  return (
    <div className="overflow-hidden rounded-[1.75rem] border border-[#17362d]/10 bg-white shadow-[0_18px_55px_rgba(36,57,47,0.08)]">
      ...
      <div className="p-6 sm:p-9">
        ...
        <div className="mt-7 grid gap-3 sm:grid-cols-2"><Button asChild className="h-11 rounded-full bg-[#254d3f] text-white"><Link href="/schedule">Book another class</Link></Button><Button asChild variant="outline" className="h-11 rounded-full bg-transparent"><Link href="/dashboard/bookings"><CalendarCheck className="size-4" />View my bookings</Link></Button></div>
      </div>
    </div>
  );
}
