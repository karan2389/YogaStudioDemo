        ...
        <aside className="lg:sticky lg:top-[108px] lg:self-start">
          <div className="rounded-[1.5rem] bg-[#17362d] p-4 text-white">
            ...
            <nav aria-label="Instructor dashboard" className="flex gap-2 overflow-x-auto pb-1 lg:flex-col lg:overflow-visible">
              {nav.map(({ href, label, icon: Icon }) => { const active = href === "/instructor" ? pathname === href : pathname.startsWith(href); return <Link key={href} href={href} className={`flex shrink-0 items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold transition ${active ? "bg-[#f8f1e7] text-[#17362d]" : "text-[#c5d2cb] hover:bg-white/8 hover:text-white"}`}><Icon className="size-4" />{label}</Link>; })}
            </nav>
          </div>
        </aside>
